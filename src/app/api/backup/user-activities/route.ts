 ...
 *   get:
 *     summary: Get user activities (Backup)
 *     description: Retrieve recent user activities from backup system
 *     parameters:
 *       - in: query
 *         name: limit
 *         schema:
...
export async function GET(request: Request) {
  try {
    ...
    const formattedActivities = activities.map(activity => ({
      ...
      userName: activity.user?.name || 'Utilisateur inconnu',
 ...
 * /api/backup/user-activities:
 *   post:
 *     summary: Create new user activity (Backup)
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function POST(request: Request) {
  try {
    ...
    // Récupérer l'adresse IP de la demande si disponible
    ...
    return NextResponse.json(
      { 
        ...
        userName: newActivity.user?.name || 'Utilisateur inconnu',

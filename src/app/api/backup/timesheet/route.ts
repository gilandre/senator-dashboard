...
const createTimesheetEntrySchema = z.object({
  ...
  start_time: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)(:[0-5]\d)?$/, {
    message: "Format horaire invalide (HH:MM ou HH:MM:SS requis)"
  }),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const startTimeObj = new Date(`1970-01-01T${start_time}Z`);
    const endTimeObj = new Date(`1970-01-01T${end_time}Z`);
